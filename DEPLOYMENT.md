...
* Deployment of our  application to Heroku. This step consists of 3 main parts:
...
```sh
conda create -n js_overload_env python=3.7
```

* Make sure to activate this new environment before proceeding.

```sh
source activate js_overload_env
```
...
* `dash_app` is the name of the folder that contains our app as a python package (i.e. the name of the folder with the `__init__.py` file in it).
